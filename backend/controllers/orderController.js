...
const shippingFor = async (city, subtotal) => {
  ...
  return (cfg.localCities || [])
    .map((x) => x.toLowerCase())
    .includes((city || "").toLowerCase())
    ? Number(cfg.localCharge || 0)
    : Number(cfg.otherCharge || 0);
};
...
export const createOrder = async (req, res) => {
  ...
  for (const item of items) {
    ...
    if (!product || !product.isActive) {
      return res.status(400).json({
        message: "One product is unavailable",
      });
    }
    ...
    if (!variant || variant.stock < Number(item.quantity)) {
      return res.status(400).json({
        message: `${product.name} selected variant is out of stock`,
      });
    }

    subtotal +=
      product.sellingPrice * Number(item.quantity);
  ...
  let shippingAmount = await shippingFor(
    address?.city,
    subtotal - discount
  );
...
// Lets the frontend show a live, accurate breakdown
// (shipping + discount + total) on the Checkout page
// BEFORE the order is placed — same pricing logic as
// createOrder, but read-only: no stock deduction,
// no coupon usage increment.
...
export const getOrderQuote = async (req, res) => {
  ...
  if (couponCode) {
    ...
    if (
      ...
      couponMessage =
        "This coupon isn't valid for this order";
...
export const customerUpdateOrder = async (
  req,
  res
) => {
  ...
  if (req.body.action === "cancel") {
    ...
    o.timeline.push({
      status: o.status,
      note:
        "Delivery address updated by customer",
    });
  } else {
    return res.status(400).json({
...
// Builds the store info block for invoices from
// the admin-configurable "store" setting,
// with sensible fallbacks so invoices never break
// if the setting hasn't been configured yet.
